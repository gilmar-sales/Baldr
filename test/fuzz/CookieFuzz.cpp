...
namespace
{
                                ...
                                const baldr::CookieOptions& opts)
    {
        ...
        {
            case baldr::SameSite::None:
                out += "; SameSite=None";
                break;
            case baldr::SameSite::Lax:
                out += "; SameSite=Lax";
                break;
            case baldr::SameSite::Strict:
                out += "; SameSite=Strict";
                break;
        }
        return out;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, std::size_t size)
{
    ...
    BDR_RECORD(fdp, cookie, "cookie_raw");
    ...
    for (const auto& [name, _] : status.request.cookies)
    {
        ...
        if (bad)
        {
            BDR_RECORD(fdp, name, "cookie_name");
            BDR_ASSERT(rec, false,
                       "parser produced cookie name that violates "
                       "RFC 6265 token grammar");
        }
    }
    ...
    BDR_RECORD(fdp, serialised, "serialised");

    BDR_ASSERT(rec,
               serialised.find('\r') == std::string::npos &&
                   serialised.find('\n') == std::string::npos,
               "serialised Set-Cookie contains CR or LF");
