...
TEST_F(RouteOptionsSpec, InsertWithOptionsCarriesAllFields)
{
    ...
    EXPECT_EQ(*e.options.description,
    ...
    EXPECT_EQ(e.method, HttpMethod::Get);
}
...
TEST_F(RouteOptionsSpec, SnapshotWalksEveryRegisteredMethod)
{
    ...
    std::set<HttpMethod> seen;
    for (const auto& e : entries)
        seen.insert(e.method);
    EXPECT_TRUE(seen.contains(HttpMethod::Get));
    EXPECT_TRUE(seen.contains(HttpMethod::Post));
    EXPECT_TRUE(seen.contains(HttpMethod::Delete));
}
...
TEST_F(RouteOptionsSpec, MatchResultExposesRouteTemplate)
{
    ...
    auto result = mRouter->matchWithAllow(HttpMethod::Get, "/users/42");
