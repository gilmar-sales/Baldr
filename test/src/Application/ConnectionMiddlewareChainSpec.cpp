...
class FakeMiddleware : public baldr::IMiddleware
{
  public:
    explicit FakeMiddleware(std::function<void(baldr::NextMiddleware)> body) :
        mBody(std::move(body))
    {
    }

    void Handle(baldr::HttpRequest&          request,
  ...
  private:
    std::function<void(baldr::NextMiddleware)> mBody;
};
...
class HttpConnectionMiddlewareChainSpec : public ::testing::Test
{
  protected:
    ...
    static baldr::MiddlewareFactory makeFactory(
        std::function<void(baldr::NextMiddleware)> body)
    {
       ...
       ShouldRunAllMiddlewaresInOrderBeforeFinalHandler)
{
    ...
    factories.push_back(makeFactory([&](baldr::NextMiddleware next) {
        int id = ++counter;
        order.push_back(id);
        next();
    }));
       ...
       ShouldShortCircuitWhenMiddlewareSkipsNext)
{
    ...
    factories.push_back(makeFactory([&](baldr::NextMiddleware) { ++counter; }));
    factories.push_back(makeFactory([&](baldr::NextMiddleware) { ++counter; }));
       ...
       MiddlewareObservesRequestFieldsBeforeAndAfterNext)
{
    ...
    baldr::HttpRequest  seenRequest;
    baldr::HttpResponse seenResponse;
    std::atomic<bool>   nextReturned { false };

    factories.push_back(makeFactory([&](baldr::NextMiddleware next) {
        // Pre-next snapshot
        EXPECT_EQ(mRequest.method, baldr::HttpMethod::Get);
        ...
        // Post-next: request must be unchanged (baldr::HttpRequest is taken by
        // const&)
        EXPECT_EQ(mRequest.method, baldr::HttpMethod::Get);
        EXPECT_EQ(mRequest.path, "/api/test");
    }));
       ...
       MiddlewareObservesHandlerMutationAfterNext)
{
    ...
    baldr::RouteHandler finalHandler =
        [&](baldr::HttpRequest&, baldr::HttpResponse& response,
            const skr::Arc<skr::ServiceProvider>&) {
       ...
       MiddlewareObservesHandlerMutationEvenWithNestedMiddlewares)
{
    ...
    EXPECT_EQ(innerAfterNext.load(), static_cast<int>(baldr::StatusCode::OK));
    // Outer middleware: next() awaits everything below (inner + handler),
    // so it also sees OK and the handler's body.
    EXPECT_EQ(outerAfterNext.load(), static_cast<int>(baldr::StatusCode::OK));
       ...
       MiddlewareCanMutateResponseBeforeFinalHandler)
{
    ...
    factories.push_back(makeFactory([&](baldr::NextMiddleware next) {
        mResponse.statusCode       = baldr::StatusCode::Accepted;
    ...
    EXPECT_EQ(seenStatus.load(), static_cast<int>(baldr::StatusCode::Accepted));
...
{
    // The default baldr::HttpResponse(request) constructor sets statusCode to
       ...
       MiddlewareCanShortCircuitAndSetResponse)
{
    ...
    factories.push_back(makeFactory([&](baldr::NextMiddleware) {
        mResponse.statusCode = baldr::StatusCode::Unauthorized;
    ...
    EXPECT_FALSE(handlerRan.load());
    EXPECT_EQ(static_cast<int>(mResponse.statusCode),
              static_cast<int>(baldr::StatusCode::Unauthorized));
