...
- OpenAPI `requestBody` rendering for handlers that declare a body (via `FromBody<T>`, `WithRequestType<T>()`, or `WithRequestSchemaJson`). The generator emits `requestBody.{required,content.<mime>.schema}` between `parameters` and `responses`. Defaults: `required: false`, media type `application/json`. New builder methods: `RouteRegistration::WithRequestBodyContentType(std::string mime)` to override the MIME (e.g. `application/x-www-form-urlencoded`) and `RouteRegistration::WithRequestBodyRequired(bool = true)` to flip the `required` flag.
- `EmitStructSchema` now describes `std::optional<U>` fields with the inner type (e.g. `std::optional<std::string>` → `{"type":"string"}`) instead of `{"type":"null"}`, and excludes them from `required`. Non-optional members remain required.
