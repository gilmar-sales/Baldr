...
namespace BALDR_NAMESPACE
{
                                       ...
                                       const NextMiddleware& next)
    {
        ...
        if (acceptIt != request.headers.end())
        {
            ...
            while (pos < ae.size())
            {
                ...
                while (a < b &&
                       std::isspace(static_cast<unsigned char>(token[a])))
                    ++a;
                while (b > a &&
        ...
        if (!Detail::gzipCompress(response.body, compressed, mOptions.level))
            return;
        ...
        const std::unordered_set<std::string>& prefixes)
    {
        ...
        while (!lc.empty() &&
            ...
            lc.erase(lc.begin());
        while (!lc.empty() &&
               std::isspace(static_cast<unsigned char>(lc.back())))
            lc.pop_back();
        ...
        for (const auto& prefix : prefixes)
        {
            ...
            {
                match =
                    lc.size() >= p.size() && lc.compare(0, p.size(), p) == 0;
            }
            else
            {
                match = lc == p;
