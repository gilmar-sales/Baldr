...
ParseResult parseCore(std::string_view buffer, std::size_t maxBodySize)
{
    ...
    std::string_view methodView(buffer.data() + methodStart, pos - methodStart);
    ...
    if (pos == versionStart || buffer[pos] != '\r' || pos + 1 >= headerEnd ||
        buffer[pos + 1] != '\n')
    {
        out.error      = "HTTP version is missing or invalid";
        out.statusCode = StatusCode::BadRequest;
    ...
    while (pos < headerEnd)
    {
        ...
        while (pos < headerEnd && buffer[pos] != '\r')
            ++pos;
        if (pos >= headerEnd || pos + 1 >= headerEnd || buffer[pos + 1] != '\n')
        {
        ...
        if (key == "content-length")
        {
            ...
            {
                out.error = "Content-Length exceeds maximum allowed body size";
                out.statusCode = StatusCode::BadRequest;
                return out;
            }
    ...
    {
        out.error = "Conflicting Content-Length and Transfer-Encoding headers";
        out.statusCode = StatusCode::BadRequest;
        return out;
    }
