...
namespace BALDR_NAMESPACE
{
    ...
    HttpServer::HttpServer(
        const skr::Arc<HttpServerOptions>&       httpServerOptions,
        ...
        mImpl(std::make_unique<Impl>())
    {
        ...
        mImpl->mResolvedThreadCount = std::max(
            1, resolveThreadCount(mImpl->mHttpServerOptions->threadCount));
    }
    ...
    void HttpServer::Run()
    {
        ...
        int timeoutSec =
            mImpl->mHttpServerOptions->gracefulShutdownTimeoutSeconds;
        bool immediate = timeoutSec < 0;
        if (mImpl->mInFlightTracker &&
            mImpl->mInFlightTracker->outstanding() > 0 && !immediate)
        {
            mImpl->mLogger->LogInformation(
