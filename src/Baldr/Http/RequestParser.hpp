...
struct HttpParseStatus
{
    ...
    std::string errorMessage;             ///< Human-readable error (populated on @c Error).
    StatusCode  statusCode    = StatusCode::OK; ///< Suggested status code on @c Error.
    std::size_t consumedBytes = 0;        ///< Bytes consumed from the buffer on @c Complete.
};
